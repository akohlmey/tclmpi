...
if {$rank == $master} {puts "startup time: [expr {([clock clicks]-$tstart)/1000000.0}] seconds"}
set tstart [clock clicks]
...
set mypi [expr {$h * $sum}]
if {$rank == $master} {puts "loop time:    [expr {([clock clicks]-$tstart)/1000000.0}] seconds"}
set tstart [clock clicks]
...
if {$rank == $master} {puts "result time:  [expr {([clock clicks]-$tstart)/1000000.0}] seconds"}
